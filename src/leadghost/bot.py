...
class Bot(SeleniumBot):
    ...
    def _run(self):
        self.bot_print(f"{self.search} @ {self.location} [<{self.max_company_size}] [{self.job_count}]")
        ...
        if self.automatic:
            ...
            while not self.script("""return document.querySelector('#runLM')"""):
                ...
                span.innerHTML = '<button id="runLM"'
                    + ' style="background-color: #2db0ad;margin: 0 auto;display: block;"'
                    + ' class="artdeco-button artdeco-button--3 artdeco-button--primary'
                    + ' ember-view mt4"><span class="artdeco-button__text">'
                    + 'Run LeadMonster</span></button>';
        ...
        while len(self.scraped_postings) < self.job_count:
            self.bot_print(
                f"{self.search} @ {self.location}"
                f" [<{self.max_company_size}]"
                f" [{len(self.scraped_postings)}/{self.job_count}]"
                f" [Page {page}]"
            )
            for _ in range(4):
            ...
            for idx, _card in enumerate(cards):
                self.click(self.css(self.JOB_CARD, getall=True)[idx])
                time.sleep(1.5)
                self.parse_posting()
        ...
        filename = os.path.join("data", f"{len(self.lead_list)}_{self.search}_{self.location}.csv")
    ...
    def average_company_size(company_size):
        ...
        sizes = [int(i) for i in re.findall(r"\d+", cs) if i.isdigit()]
        if not sizes:
            return 1
    ...
    def bot_print(self, message, input_msg=None, figlet=False):
        if figlet:
            ...
            self.c.print(f"[bold blue][{__NAME__} {__VERSION__}][/bold blue] {message}")
